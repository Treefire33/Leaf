...
// stolen from: raylib-cs example: rectangle bounds
		...
		Color selectBackTint)
	{
		...
		{
			length = (int)Raylib.TextLength((sbyte*)p); // Total length in bytes of the text, scanned by codepoints in loop
		}
		...
		for (int i = 0, k = 0; i < length; i++, k++)
		{
			...
			else
			{
				...
				else
				{
					...
					{
						Raylib.DrawRectangleRec(new Rectangle(rec.X + textOffsetX - 1, rec.Y + textOffsetY, glyphWidth, (float)font.BaseSize * scaleFactor), selectBackTint);
					...
					{
						Raylib.DrawTextCodepoint(font, codepoint, new System.Numerics.Vector2(rec.X + textOffsetX, rec.Y + textOffsetY), fontSize, isGlyphSelected ? selectTint : tint);
					}
				}
